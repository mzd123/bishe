				...
				<tbody>
					<c:forEach items="${sessionScope.li }" var="each" varStatus="st">
						<tr>
							...
							<td>${each.fnicheng }</td>
							<td><a
								href="<%=basePath%>selectmyfriendbynicheng.do?op=2&fnicheng=${each.fnicheng }">点击查看</a></td>
							<td><a
								href="<%=basePath%>deletetmyfriendbynicheng.do?fnicheng=${each.fnicheng }">点击删除</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
